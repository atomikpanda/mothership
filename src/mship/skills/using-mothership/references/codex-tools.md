...
- any explicit value: pass it unchanged through a supported model selector.
- `inherit`: no explicit value was configured — do NOT dispatch with the
  selector simply omitted, silently inheriting whatever the session runs
  (usually its most expensive model). Choose a deliberate tier per the
  executing skill's Model Selection rules and set it explicitly on the
  spawn.
...
V1 multi-agent APIs expose no model selector, so they must reject an
explicit resolved value with that actionable message. V2 `spawn_agent`
supports `model` and `reasoning_effort`; supply explicit values through
it unchanged.
...
explicitly: the stub's resolved model when one is given, otherwise the
tier the executing skill's Model Selection rules name. Setting `model`
alone is a trap: the child's effort silently resets to that model's
default, not to yours.
