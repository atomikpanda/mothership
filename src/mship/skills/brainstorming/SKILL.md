...
- **Spike** — a feasibility question ("can we...", "is it possible...",
  ...
  doc, no spec. Report findings as a recommendation; anything you
...
- **Bounded** — a well-scoped change to code that already exists in
  ...
  one. No spec, no implementation plan document.
- **Architectural** — new projects, new subsystems, changes that
  restructure how components fit together or alter interfaces others
  depend on. Follow the full process: questions, approaches, sectioned
  design, captured spec, then the writing-plans skill.

When in doubt between two paths, take the heavier one. The ratchet is
one-way: hidden complexity discovered mid-task upgrades the path —
stop, say so, and step up. Nothing downgrades mid-task.

**In a mothership workspace, classification does not bypass mship's
gates.** Every task still needs a WorkItem anchor before `mship spawn`:
bounded work maps to a chore/bug-kind WorkItem (no spec required);
feature-kind WorkItems hard-require an approved `mship spec` regardless
of how simple the change looks — if the work is a feature, it takes the
architectural path's spec capture. A spike needs no WorkItem until you
...
| "It's a feature, but tiny — chore WorkItem and skip the spec" | Kind follows what the work IS, not how big it feels. Feature-kind means spec; mislabeling to dodge the gate is the doubt itself. |
...
5. **Implement** — proceed with the normal development workflow (TDD applies); no plan document. In a mothership workspace, anchor first: `mship item new --kind <chore|bug>` → `mship spawn --work-item`.
