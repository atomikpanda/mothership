...
This is enforced everywhere a task's identity matters, not just at `spawn`: the `git commit`/`git push` hooks and the PreToolUse edit-guard hook all check that the active task carries a passing WorkItem gate, and refuse (with an actionable message) if it doesn't. `MSHIP_BYPASS_GATE=1` bypasses only that WorkItem/spec gate; `MSHIP_ALLOW_MAIN_EDIT=1` currently exits before PreToolUse event parsing and therefore bypasses the entire edit guard, including WorkItem/spec checks. Both require explicit operator authorization. Note `--no-verify` disables **every** pre-commit check, not just this gate. See [Work items: the cross-artifact spine](#work-items-the-cross-artifact-spine-mship-item) above for the full WorkItem model.
