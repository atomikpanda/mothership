...
**Upstream base:** `obra/superpowers v6.3.0` (commit `b36e0829c6d0140e93cfef2ca599b1b07d4a7797`)
**Vendored:** 2026-07-28 at v6.2.0 (spec `re-vendor-superpowers-620-with-mship`, mothership #437/#439); base bumped to v6.3.0 on 2026-08-18 by selectively integrating its skill changes (brainstorming three-path router, SDD rulings-not-stalls + no-subagents contracts, Codex multi-agent V2 guidance, misc fixes) into the re-woven files below. v6.3.0's Devin/Hermes plugin infrastructure is out of scope (see structural exclusions).
...
- **`skills/using-superpowers/`** (except `references/`) — not vendored; `using-mothership` is its role-equivalent. Its `references/*` ARE vendored, relocated to `using-mothership/references/` (the manifest records them at the relocated path) — EXCEPT `references/hermes-tools.md` (new in v6.3.0), which is not vendored: `mship skill install` targets claude/codex/gemini, not Hermes Agent. Likewise none of v6.3.0's Devin (`.devin-plugin/`) or Hermes (`.hermes-plugin/`) plugin infrastructure is vendored.
...
- `finishing-a-development-branch/SKILL.md` — kind 2: PR path replaced by `mship finish --body-file` (push+PR+state stamp, evidence-gated by default; `--require-tests` reference dropped as a deprecated no-op, task `mship-finish-requires-test-evidence-by`) with post-finish iteration via `mship commit`; Step 1 verification runs `mship test` (not a bare runner) so finish's evidence gate has something to read; merge path corrected (intent-scan + PR #443 review) — Option 1 is not the normal workspace path (Option 2 + merge auto-advance is); the sanctioned local-merge-no-PR sequence is `mship finish --push-only` → merge EVERY affected repo's branch into its base → plain `mship close`, in that order (close verifies each repo's merge reached its base and only then advances spec/WorkItem like a merged close; a partially-merged close prompts before teardown and does not advance; closing before any merge is safe but advances nothing), with a runnable per-repo merge block driven by `mship status` jq state (iterates `affected_repos` looked up in the worktrees map — passive dependency checkouts are deliberately never merged) that resolves each repo's OWN base with mship's real precedence — task `base_override` (stacked `--base`) > repo `base_branch` from mothership.yaml > task default — via python3+yaml with override/repo/fallback passed as argv (zero shell interpolation into the python source); anchored to an explicit `--task "$TASK"` so multi-task workspaces resolve; fail-fast — the loop exits 1 at the first failed checkout/pull/merge so it never continues past a conflicted checkout (re-running skips already-delivered repos); runs from the workspace root; upstream's single-repo block kept for non-workspace use — never `close --abandon` for delivered work; discard path routes through `mship close --abandon`. Non-workspace flows keep upstream's forge tooling.
...
- `subagent-driven-development/SKILL.md` — kind 3: pre-dispatch `mship status` envelope checks (empty `active_tasks` → WorkItem-first stop; unanchored multi-task → pick and `--task`), worktree-only work contract, cross-reference to `working-with-mothership`'s authoritative CLI contract. v6.3.0 rulings-not-stalls autonomy integrated on journal vocabulary: rulings recorded as `mship journal "Ruling: ..."` (ledger → journal throughout), the four stop classes routed through an `mship ask` decision card (operator memory: blocking asks must reach the phone), the preflight conflict-scan table journaled one entry per row, spec located via the plan's Spec header or the task's WorkItem (`mship view spec`), same-shape task batching via `mship dispatch -i -` with the batch brief journaled, bounded-stretch waiting, the implementer no-subagents contract note, the pre-finish "Rulings I made" roll-up (collected from `mship journal --last 100` because `mship close` removes task state), and the worker-spawned-reviewer red-flag row. Model Selection `inherit` re-specified per operator directive (2026-08-18): where the subagent API has a selector, set model AND effort explicitly per the task-complexity rules — never dispatch with the selector omitted; no-selector harnesses keep the harness default.
...
- `test-driven-development/SKILL.md` — kind 4: run tests via `mship test` (records the evidence `mship finish` requires by default, task `mship-finish-requires-test-evidence-by`); open debug threads auto-attach `parent=<hypothesis-id>` to test-run journal entries, cross-referencing `systematic-debugging`.
...
- `using-mothership/references/codex-tools.md` — kind 3: dispatch-resolved `inherit` omits the model selector; explicit model values pass through verbatim or produce the exact unsupported-selector error.- `using-mothership/references/codex-tools.md` — kind 3, rebased on v6.3.0's multi-agent V1/V2 rewrite (fork_turns/agent_type spawning, `followup_task` fix rounds, V2 auto-eviction, allowlist-checked model names, `wait_agent` bounded-stretch waiting, the config.toml subagent backstop). Resolved-model delta re-scoped for V2 selectors + operator directive (2026-08-18): explicit resolved values pass through `spawn_agent`'s `model`/`reasoning_effort` unchanged; `inherit` no longer omits the selector where one exists — choose a deliberate tier per the executing skill's Model Selection rules and set model AND effort explicitly (upstream's always-explicit routing rule kept, re-pointed at the stub's resolved model first); the exact unsupported-selector error is retained for selector-less (V1) APIs.
...
- `verification-before-completion/SKILL.md` — kind 4: "Mothership Workspace" section — verify via `mship test` so the result is recorded as evidence for the `mship finish` gate (enforced by default, task `mship-finish-requires-test-evidence-by`); controller-role paragraph added (intent-scan) — in subagent-driven flows the controller may accept a subagent's reported `mship test` evidence once a reviewer verified the work; own-work claims still need fresh self-run evidence.

### writing-plans

- `writing-plans/SKILL.md` — kind 3 + kind 2 + residual kind 1: plan input is an approved `mship spec` (id in the plan header, `mship phase dev` gate); save path parametrized on `docs_dir` from `mship context`; `<!-- mship:task id=N -->` anchors around plan tasks for `--plan-task` dispatch; commit steps paired with `mship journal`; remaining upstream skill-namespace (`superpowers`) prefixes dropped. Plan-gate warning added at the save-location line (intent-scan): filename must stem-match the task slug or be linked via `mship item link-plan`, else the feature plan gate hard-blocks; canonical finish reference now plain `mship finish` (evidence gate is on by default, task `mship-finish-requires-test-evidence-by`). v6.3.0 **Spec:** header field adopted, bound to the approved `mship spec` id plus the spec file path (upstream binds a doc path only).
...
- `writing-skills/render-graphs.js` — v6.3.0 `execFileSync` hardening and Windows-safe `dot -V` availability probe adopted; upstream's CJS→ESM import conversion NOT adopted (kept `require()` — the vendored tree ships no package.json module context, so ESM in a `.js` entrypoint would break `./render-graphs.js`).
