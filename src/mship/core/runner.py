...
class RunDeps:
    ...
    run_state: object                               # RunStateRepo-like: try_claim/release/append_log
...
def run_once(deps: RunDeps) -> RunOnceResult | None:
    ...
    """
    candidates = select_runnable(deps.items, deps.spec_approved, deps.claimed)
...
def checkpoint_bail(deps: RunDeps, item, reason: str) -> None:
    """Bail out of a claimed item: log the reason, mark it blocked, release the claim.

    Called by the host when the run cannot make progress (an exception, a fork in the
    approach, a needed decision). Order matters: the reason is logged first so it is
    durable even if the block/release races, then the item is marked blocked (so the
    selector won't immediately re-pick it), then the claim is released so the backlog
    can move on. Never merges — the branch is left intact for a later resume.
    """
    deps.run_state.append_log(item.id, f"bailed: {reason}", deps.now())
    deps.mark_blocked(item, reason)
    deps.run_state.release(item.id, deps.holder)
