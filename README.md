...
**Isolation that makes coordination safe.** Every worktree is on its own feature branch, separate from main. A pre-commit hook refuses commits from outside the active task's worktrees. `mship init` and `mship init --install-hooks` also install lifecycle integrations for Claude Code (`.claude/settings.json` at the workspace root) plus Codex (`.codex/hooks.json`) and OMP (`.omp/extensions/mship.ts`) in every configured Git root. Commit the project-local Codex and OMP artifacts so spawned worktrees inherit them. All three inject Mothership session context, guard edits against the active worktree and WorkItem policy, and continue once for actionable inbox work. `MSHIP_ALLOW_MAIN_EDIT=1` currently bypasses the entire PreToolUse edit guard, including WorkItem/spec checks; Codex still requires explicit project-hook review through `/hooks`.
