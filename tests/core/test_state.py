...
def test_save_and_load_roundtrip(state_dir: Path):
    ...
    mgr.save(state)
    loaded = mgr.load()
    assert "add-labels" in loaded.tasks
...
def test_save_with_test_results(state_dir: Path):
    ...
    state = WorkspaceState(tasks={"fix-auth": task})
    mgr.save(state)
    loaded = mgr.load()
...
def test_loads_legacy_current_task_field_as_extra_ignore(state_dir: Path):
    """Legacy state.yaml files with current_task: should still parse cleanly."""
    import yaml
    legacy_yaml = {
    ...
    (state_dir / "state.yaml").write_text(yaml.safe_dump(legacy_yaml))
    mgr = StateManager(state_dir)
    state = mgr.load()
    # current_task silently dropped by extra="ignore"; tasks load cleanly.
    assert not hasattr(state, "current_task") or getattr(state, "current_task", None) is None
    assert "fix-auth" in state.tasks
...
def test_task_blocked_roundtrip(state_dir: Path):
    ...
    state = WorkspaceState(tasks={"test": task})
    mgr.save(state)
    loaded = mgr.load()
