...
def test_set_host_then_connection_for_role_roundtrips(tmp_path: Path):
    ...
    store.set_host(_host("ios-sim-host", "http://10.0.0.5:8787", "secret-tok"), scope="project")
    assert store.connection_for_role("ios-sim-host", environ={}) == RunHostConnection("http://10.0.0.5:8787", "secret-tok")
...
def test_env_override_wins_over_one_allowed_host(tmp_path: Path, monkeypatch):
    store = RunHostStore(tmp_path)
    store.set_host(_host("ios-sim-host", "http://file-url", "file-token"), scope="project")
    ...
    assert store.connection_for_role("ios-sim-host", environ=os.environ) == RunHostConnection("http://env-url", "env-token")
...
def test_env_only_registration_remains_supported(tmp_path: Path, monkeypatch):
    ...
    assert store.connection_for_role("android-emu-host", environ=os.environ) == RunHostConnection("http://emu", "emu-token")
...
def test_safe_hosts_never_include_tokens(tmp_path: Path):
    ...
    store.set_host(_host("android-emu-host", "http://h2", "also-secret"), scope="project")
    safe = store.safe_hosts()
    assert sorted((name, host["url"]) for name, host in safe.items()) == [("android-emu-host", "http://h2"), ("ios-sim-host", "http://h1")]
